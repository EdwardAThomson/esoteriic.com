...
I have been building a roguelike whose entire world lives on-chain. The dungeons, the segments of the overworld, who owns what, every player's hit points: all of it is game state, maintained by a GSP (a game state processor) that folds moves in from the chain and lets clients read the result back out. That GSP model comes from [Xaya](https://xaya.io), whose framework is what makes a fully on-chain game like this practical. It is appealing for the usual on-chain reasons (no central server can quietly rewrite your character, the rules are the rules), but it makes one thing genuinely hard: testing the multiplayer.
...
*The stack is open source. The on-chain GSP and game logic live in [xaya-roguelike](https://github.com/EdwardAThomson/xaya-roguelike); the frontend and the self-play harness (`agentcore.mjs`, `multi.mjs`, `compete.mjs`) live in [xaya-roguelike-frontend](https://github.com/EdwardAThomson/xaya-roguelike-frontend). Both are built on the game-channel and GSP tech from the [Xaya](https://xaya.io) team.*
