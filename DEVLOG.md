...
Published "Reviewing 23 Open-Source AI Coding Agents by Reading the Source," a write-up of a companion project (agent-reviews) that assessed 23 open-source coding agents (Aider, Cline, Goose, OpenHands, Codex CLI, Gemini CLI, and others) by reading their source at a pinned commit rather than running a benchmark. The post lays out the three-tier method: Tier 1 is mechanically extractable facts (license, LOC, dependency count), Tier 2 is evidence-based capability findings tied to specific files and lines (architecture, tool orchestration, security boundaries), and Tier 3 is opinion, but only when backed by a specific Tier 2 finding. It surfaces what that method catches that a benchmark score can't: license traps (AutoGPT's non-compete PolyForm Shield, CLIO's GPL-3.0), vendor lock-in hidden under "open" branding (single-provider enums in Codex CLI and Gemini CLI vs. genuinely provider-agnostic Cline and Goose), sustainability risk (bus-factor-of-one projects, a tool with no commits since October 2025), and security postures that are candid rather than reassuring (QM's SECURITY.md admitting its scanner is "a speed bump"). The stated conclusion is that there's no single best agent, only different tradeoffs for different users and constraints. As a pure content addition under `src/markdown/`, the build picks it up with no engine or template changes.

**Decisions & notes:** The post is explicit that reading source code can't measure runtime performance under adversarial tasks, that's left to a separate benchmark-harness project referenced as "next up." Continues the blog's cadence of writing up work from the author's other projects.
